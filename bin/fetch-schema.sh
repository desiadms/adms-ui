...
if [[ -z "$HASURA_ENDPOINT" ]]; then
  HASURA_ENDPOINT=$(bun run netlify env:get HASURA_ENDPOINT | grep -v "bun\|netlify\|Done")
fi
...
if [[ -z "$HASURA_ADMIN_SECRET" || -z "$HASURA_ENDPOINT" ]]; then
    echo "Environment variables not set. Exiting."
    exit 1
fi

# Otherwise, introspect the GraphQL endpoint
bunx graphqurl "$HASURA_ENDPOINT" --header "X-Hasura-Admin-Secret: $HASURA_ADMIN_SECRET" --introspect > schema.graphql